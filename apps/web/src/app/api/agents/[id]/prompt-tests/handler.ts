...
function readMemoryRecords(input: unknown, layer: 'short_term' | 'fixed'): MemoryRecord[] {
  const items = isRecord(input) && Array.isArray(input.memories) ? input.memories : [{
    displaySummary: layer === 'short_term' ? '用户刚刚提到星际2是喜欢的游戏。' : '用户喜欢科幻即时战略游戏。',
  ...
  return items.flatMap((item, index) => {
    ...
    const displaySummary = readText(item.displaySummary ?? item.summary).trim()
    const retrievalText = readText(item.retrievalText ?? item.retrieval_text ?? displaySummary).trim()
...
function renderPromptTest(agent: AgentRecord, body: Required<Pick<PromptTestRunInput, 'testId'>> & PromptTestRunInput) {
  ...
  if (testId === 'personality.systemPrompt') {
    return Response.json({ testId, mode: 'render', renderedOutput: prompt ?? readText(agent.systemPrompt, `You are ${agent.name}.`) })
  }
