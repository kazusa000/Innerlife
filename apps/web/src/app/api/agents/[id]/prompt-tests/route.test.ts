...
test('default prompt test inputs are generated by the backend and accepted by every registered test', async () => {
  ...
  const llmOutputs: Record<string, string> = {
    'memory.timeAnalyzer': '{"time_range":{"start":"2026-04-29T18:00:00+02:00","end":"2026-04-29T22:00:00+02:00"}}',
    'memory.semanticAnalyzer': '{"retrieval_query":"星河2叫什么名字"}',
    'memory.contextToShortTerm': '{"memories":[{"detail":"原文说明用户喜欢星河2，Stage A 应保留 surface“星河2”。","retrieval_text":"用户说星河2是自己喜欢的游戏。","importance":0.7}]}',
    'memory.entityMention': '{"mentions":[{"surface":"星河2","type":"object","context_hint":"用户提到的游戏简称","confidence":0.91}]}',
    'memory.episodicExtraction': '{"entities":[{"local_entity_id":"local-game-1","surface":"星河2","type":"object","context_hint":"用户提到的游戏简称"}],"episodic_memories":[{"summary":"用户最喜欢的游戏是星河2。","detail":"我最喜欢的游戏是星河2。","importance":0.82,"entity_links":[{"local_entity_id":"local-game-1","weight":0.9}]}]}',
    'memory.entityResolution': '{"resolutions":[{"local_entity_id":"local-game-1","action":"merge","entity_id":"entity-sc2","confidence":0.9,"alias_to_add":"星河2"}]}',
    ...
    'relationshipNamed.analysis': '{"trust_delta":0.02,"affinity_delta":0.01,"familiarity_delta":0.03,"respect_delta":0.02,"trigger":"Lin 直接反馈问题"}',
  }
  ...
  try {
    ...
    assert.match(seenInputs.get('memory.timeAnalyzer') ?? '', /当前时间/)
    assert.match(seenInputs.get('memory.timeAnalyzer') ?? '', /我昨天晚饭吃了什么/)
    assert.match(seenInputs.get('memory.semanticAnalyzer') ?? '', /我最喜欢的游戏是星河2/)
...
test('prompt test llm mode sends the sample input and parses entity mention output', async () => {
  ...
  try {
    ...
    const response = await runPromptTest(agent.id, {
      ...
      sendMessage: async (request) => {
        ...
        return {
          content: [{
            type: 'text',
            text: '{"mentions":[{"surface":"星河2","type":"object","context_hint":"游戏简称","confidence":0.92}]}',
          }],
          stopReason: 'end_turn',
    ...
    assert.equal(payload.parsedOutput.mentions[0].surface, '星河2')
    assert.equal(requests.length, 1)
    assert.equal(requests[0]?.systemPrompt, '自定义实体 mention prompt')
    assert.match(JSON.stringify(requests[0]?.messages), /星河2和云海纪元/)
  } finally {
    resetDb()
    resetMemoryDb()
    rmSync(dir, { recursive: true, force: true })
...
test('prompt test semantic analyzer keeps prior user message when building the real analyzer input', async () => {
  ...
  try {
    ...
    assert.match(inputText, /我最喜欢的游戏是星河2/)
