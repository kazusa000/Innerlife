  ...
  try {
    ...
    assert.deepEqual(agentRepo.getAgent('agent-1')?.modules, {
      memory: {
        ...
        episodicExtractionPromptByLocale: { 'zh-CN': '抽取实体和情景记忆' },
        entityResolutionPromptByLocale: { 'zh-CN': '合并实体或创建新实体' },
        shortTermFragmentPromptByLocale: { 'zh-CN': '这些是近期记忆' },
        fixedFragmentPromptByLocale: { 'zh-CN': '这些是稳定事实' },
      },
    })
  } finally {
    resetDb()
