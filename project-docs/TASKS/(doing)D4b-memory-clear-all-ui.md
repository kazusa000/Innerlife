...
### API 与数据层（可自动验证）
- [ ] 新增或扩展 collection 级删除接口，推荐 `DELETE /api/agents/[id]/memory/sqlite`。
- [ ] 接口只删除当前 `agentId` 下 sqlite memory 表里的 `short_term / long_term / fixed` 记忆，不影响其他 persona。
- [ ] 接口不删除 messages、sessions、emotion、relationship、observer、daemon event 或其他状态。
- [ ] 非 sqlite memory scheme 时返回清晰错误或禁用路径，行为与现有 memory sqlite 管理 API 保持一致。
- [ ] `memoryRepo.deleteMemoriesByAgent(agentId)` 返回删除条数，API 返回 `{ ok: true, deletedCount }` 或等价结构。
- [ ] route tests 覆盖成功清空、agent 不存在、非 sqlite scheme、不会误删其他 agent 记忆。

### UI 层（必须手动验证）
- [ ] Memory Rows 区域增加危险操作按钮，例如“清空全部记忆”。
- [ ] 点击前必须有确认弹窗，文案明确说明会删除当前 persona 的短期、长期、固化记忆。
- [ ] 成功后刷新列表、重置分页/展开行，并显示删除条数。
- [ ] 删除失败时保留现有列表并显示错误。
- [ ] 空列表状态下按钮行为合理：可以禁用，或确认后返回删除 0 条。
- [ ] typecheck 通过。
- [ ] 相关测试通过。
- [ ] 浏览器手动验证至少 2 个场景：有记忆时清空并看到列表归零；无记忆时不报错。
