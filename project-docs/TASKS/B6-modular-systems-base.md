...
- [ ] 空 modules（或 modules 全为 noop）的 agent 行为与当前完全一致（**无回归**）
- [ ] TurnContext 在一轮对话内贯穿，系统能读 / 写自己那部分 state
- [ ] promptFragments 按 priority 排序后正确拼接到 system prompt（Observer 里可验证）
- [ ] 至少写一个"玩具系统"验证链路通（比如 `hello-world` 系统，在 beforeLLM 往 promptFragments 里塞一行 `"(Debug: hello from system)"`，能在 Observer 里看到该字符串进了 system prompt）
- [ ] `packages/systems/` 能被 core + apps/web 正确引用（workspace 路径）
- [ ] registry 支持：用字符串方案名（如 `modules.emotion = "noop"`）动态实例化系统
- [ ] 更新 DESIGN 里如果发现与实际实现有分歧，记录在 task 备注里（由 Coordinator 验收时回写 DESIGN.md）
