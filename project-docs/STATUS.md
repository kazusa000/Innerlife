...
最后更新：2026-04-21（G6 图灵测试系统 v1）

---

## 一句话总结

一个能在网页上聊天、并带**本地 daemon 常驻骨架**的 AI agent，支持**创建多个虚拟人**（各有独立名称、描述、模型），当前默认可抓取网页内容，对话自动存档。
...
- **性格系统（personality:big-five）**：5 维 Big Five 滑块 + 说话风格 + 背景故事；`beforeLLM` 以 `priority: 10` 注入 system prompt，关闭（`scheme: noop` 或字段缺失）行为等同 noop。现在有固定管理入口 `/agent/[id]/personality`，支持读取 / 编辑 Big Five、说话风格和背景故事
...
- **记忆系统（memory:sqlite）**：`beforeTurn` 的查询分析已经拆成两个并行 LLM analyzer：`time analyzer` 只产出 `time_range`，`semantic analyzer` 只产出 `retrieval_query + focus`，然后在进入 embedding 检索前合并成统一 query；如果两边都产不出有效结果则直接跳过本轮记忆注入。检索支持纯时间范围过滤，命中结果注入主 prompt 时会带上 `layer + 本地时间` 前缀。`afterTurn` 让 LLM 总结本轮成 `{display_summary, retrieval_text, tags, importance}` 落独立的 `memory.db`，新写入默认 `layer=short_term`。手动触发 `POST /api/agents/:id/memory/sqlite/consolidate` 会按 layer 分组整理该 agent 的记忆（只做层内 keep / rewrite / merge，不跨层合并）。首页表单现在只配置 `memory.scheme = noop | sqlite`，固定 `/agent/[id]/memory` 入口可管理 sqlite 记忆的表格列表 / 搜索 / layer 筛选 / 删除 / 单条改 layer / consolidate，并编辑 `Memory model override`、`Embedding model`、`Time Analyzer Prompt`、`Semantic Analyzer Prompt`、`Summarize Prompt`、`Fragment Prompt`、`Consolidate Prompt`。Observer 用 `kind: 'memory'` 标记，`metadata.phase` 区分 `retrieve` / `summarize` / `consolidate`；其中 retrieve 会清楚显示 `timeAnalyzer`、`semanticAnalyzer`、`mergedQuery` 和最终 `hits`
- **图灵测试系统（Turing test v1）**：每个 persona 现在都有固定入口 `/agent/[id]/turing`。页面不是聊天页变种，而是图灵测试工作台：可发起一次**外部优先的异步评测 run**，后台会复制当前 persona 为临时测试 agent、强制开启全部模块，并由固定 6 段测试套件（自然开场 / 日常延续 / 记忆追问 / 情绪合理性 / 关系边界 / 不确定性与露馅处理）自动对话。右侧有命令行风格的后台日志台，主区显示报告，底部保留完整对话回放。测试结果默认保留，并支持一键清理本次 run 产生的临时 agent、会话、记忆、关系/情绪状态和事件日志
...
- ❌ daemon 目前已经能消费图灵测试 queued run，但 `scheduled_tasks`、Context → STM → LTM 搬运、自主行为都还没接上
