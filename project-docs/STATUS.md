...
- **记忆实体图与情景记忆**（在独立 `memory.db`）—— `memory_entities`、`memory_entity_aliases`、`memory_entity_edges`、`episodic_memories`、`episodic_memory_entities`，用于 persona 级长期情景召回
...
- ❌ daemon 目前已经能消费图灵测试 queued run，也已经接上 Context → STM 与 STM → entity graph + episodic 的记忆搬运；但 `scheduled_tasks` 与更高阶自主行为仍未实现
