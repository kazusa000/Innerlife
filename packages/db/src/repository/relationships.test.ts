...
test('upsertRelationship creates and updates the current relationship record for one counterpart', () => {
  ...
  try {
    ...
    assert.equal(getRelationship('agent-1', 'user', 'default-user'), undefined)
...
test('listRelationshipHistory preserves append order and round-trips structured deltas', () => {
  const dir = mkdtempSync(join(tmpdir(), 'mas-relationships-repo-'))
  const dbPath = join(dir, 'test.db')
