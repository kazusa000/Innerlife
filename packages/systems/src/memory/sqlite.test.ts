...
test('memory sqlite retrieval prompt guides pure recent-utterance recall toward time-only retrieval', { concurrency: false }, async () => {
  ...
  assert.match(ctx.pendingMemoryQuery?.prompt ?? '', /不要包含任何时间信息/)
  assert.match(ctx.pendingMemoryQuery?.prompt ?? '', /时间只属于 time_range/)
  assert.match(ctx.pendingMemoryQuery?.prompt ?? '', /只保留主题锚点本身/)
