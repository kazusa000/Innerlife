...
export function buildSemanticAnalyzerPrompt(promptOverride?: string | null, locale: AppLocale = 'zh-CN'): string {
  ...
  if (locale === 'en-US') {
    return [
      ...
      'After removing time and wrapper phrasing, keep concrete objects, topics, scenes, names, meals, foods, books, purchases, sleep, bugs, places, relationships, or imagery.',
      'Daily-life recall questions are valid retrieval anchors: "What did I have for dinner yesterday?" -> "my dinner"; "What book did I buy last week?" -> "the book I bought"; "How has my sleep been recently?" -> "my sleep".',
  ...
  const defaultLines = [
    ...
    '去掉时间和回顾外壳后，如果还剩下具体对象、主题、画面、名字、餐食、食物、书、购买物、睡眠、bug、地点、关系或意象，就保留它，不要误判成 null。',
    '如果原句是在回顾某个时间段里聊过的对象、场景、画面、名字或事件类型，去掉时间后剩下的那部分仍然是主题锚点。',
    '如果原句里明确出现了“画面”“场景”“名字”“地点”“餐食”“晚饭”“午饭”“早餐”“食物”“书”“买了什么”“睡眠”“bug”这类名词短语，而去掉时间后它们仍然存在，则 retrieval_query 不能为 null。',
    '如果剩下的主题本身就是一个抽象对象，但它已经明确指向用户要找的内容，例如“画面”“场景”“名字”“梦境”“氛围”，就把它补成一句短完整的话，不要直接只丢一个词，也不要返回 null。',
    '日常生活回忆也是有效主题锚点，例如“我昨天晚饭吃了什么”补成“我的晚饭是什么”，“我上周买了什么书”补成“我买的书是什么”，“我最近睡眠怎么样”补成“我的睡眠怎么样”。',
